...
{
    unsigned int index = hash(i.contents);
    
    list<Item>::iterator the_item = items[index].begin();
    
    for(unsigned int j = 0; j < items[index].size(); j++)
    {
        if(the_item->contents == i.contents)
        {
            the_item->count ++;
...
{
    unsigned int index = hash(contents);
    
    list<Item>::iterator the_item = items[index].begin();
    
    for(unsigned int i = 0; i < items[index].size(); i++)
    {
        if(the_item->contents == contents)
        {
            items[i].erase(the_item);
...
string HashTable::printDetail ()
{
    ...
    for(unsigned int i = 0; i < items.size(); i++)
    {
        ...
        list<Item>::iterator the_item = items[i].begin();
        
        for(unsigned int j = 0; j < items[i].size(); j++)
        {
            ss << setw(10) << the_item->contents << " (" << the_item->count << ") ";
