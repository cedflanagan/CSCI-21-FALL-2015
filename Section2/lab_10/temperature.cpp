 ...
 * Description : Functions for manipulating temperature.
 */
...
// CLASS FUNCTION DEFINITIONS GO HERE
  ...
  {
      double fahrenheit = kelvin_ * 1.8 - 459.67;
  ...
  string Temperature::ToString(char unit) const
  {
      ...
      switch (unit)
      {
          case 'K':
          case 'k':
            final_temp = GetTempAsKelvin();
            final_unit = "Kelvin";
            break;
          case 'C':
          case 'c':
            final_temp = GetTempAsCelsius();
            final_unit = "Celsius";
            break;
          case 'F':
          case 'f':
            final_temp = GetTempAsFahrenheit();
      ...
      stringstream ss;
      ss << std::fixed;
      ss << std::setprecision(2) << final_temp << " " << final_unit;
